...
void RawInput::EnableRawMode() {
    ...
    // VMIN=0, VTIME=1: ждать до 0.1 сек для сбора последовательности
    raw.c_cc[VMIN] = 0;
    raw.c_cc[VTIME] = 1; 
...
RawInput::ParseResult RawInput::TryParseSequence(const std::string& buffer, size_t startPos) {
    ...
    // Управляющие символы (Ctrl+буква)
    if (c >= 0x01 && c <= 0x1A) {
    ...
    if (c >= 0x20 && c <= 0x7E) {
        if (c >= 'A' && c <= 'Z') {
            // Твое требование: Shift + строчная буква
            Key lower = static_cast<Key>(c + 0x20);
            return {{Key::Shift, lower}, 1, true};
        }
        
        // Маппинг символов с Shift+цифра
        if (c == '!') return {{Key::Shift, Key::Digit1}, 1, true};
        if (c == '@') return {{Key::Shift, Key::Digit2}, 1, true};
        if (c == '#') return {{Key::Shift, Key::Digit3}, 1, true};
        if (c == '$') return {{Key::Shift, Key::Digit4}, 1, true};
        if (c == '%') return {{Key::Shift, Key::Digit5}, 1, true};
        if (c == '^') return {{Key::Shift, Key::Digit6}, 1, true};
        if (c == '&') return {{Key::Shift, Key::Digit7}, 1, true};
        if (c == '*') return {{Key::Shift, Key::Digit8}, 1, true};
        if (c == '(') return {{Key::Shift, Key::Digit9}, 1, true};
        if (c == ')') return {{Key::Shift, Key::Digit0}, 1, true};
...
RawInput::ParseResult RawInput::ParseCSI(const std::string& buffer, size_t startPos) {
    ...
    if (seq == "\x1B[H") return {{Key::Home}, seq.length(), true};
    if (seq == "\x1B[F") return {{Key::End}, seq.length(), true};
    if (seq == "\x1B[2~") return {{Key::Insert}, seq.length(), true};
    if (seq == "\x1B[3~") return {{Key::Delete}, seq.length(), true};
    if (seq == "\x1B[5~") return {{Key::PageUp}, seq.length(), true};
    if (seq == "\x1B[6~") return {{Key::PageDown}, seq.length(), true};
    if (seq == "\x1B[Z") return {{Key::Shift, Key::Tab}, seq.length(), true};

    // Модифицированные CSI (например, \x1B[1;5A = Ctrl+Up)
    if (seq.find(';') != std::string::npos) {
        ...
        try {
            ...
            if (baseKey != Key::None) {
                ...
                if (mod == 3) result = {Key::Alt, baseKey};
                if (mod == 5) result = {Key::Ctrl, baseKey};
                if (mod == 6) result = {Key::Shift, Key::Ctrl, baseKey};
...
RawInput::ParseResult RawInput::ParseSS3(const std::string& buffer, size_t startPos) {
    ...
    std::string seq = buffer.substr(startPos, 3);
